...
def _load_embeddings(file, vocab, dim, lower, mode='freq'):
...
def load_embeddings(args, config, vocab):
    """generate embeddings suited for the current vocab or load previously cached ones."""
    embedding_file = os.path.join(os.path.dirname(__file__), 'embedding.msgpack')
    if not os.path.exists(embedding_file):
        logger.info(f"loading embedding from {args.pretrained_embeddings}")
        embeddings = _load_embeddings(args.pretrained_embeddings, vocab,
                                      config.embedding_dim, mode=args.embedding_mode,
                                      lower=args.lower_case)
        with open(embedding_file, 'wb') as f:
            msgpack.dump(embeddings, f)
    else:
        logger.info(f"loading embedding from {embedding_file}")
